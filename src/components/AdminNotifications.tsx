    ...
    <div className="relative font-sans antialiased text-slate-900" ref={dropdownRef}>
      ...
      {isOpen && (
        <>
          {/* Mobile backdrop */}
          <div 
            className="fixed inset-0 bg-black/30 backdrop-blur-2xs z-40 sm:hidden transition-opacity"
            onClick={() => setIsOpen(false)}
          />

          <div className="fixed left-2 right-2 top-14 sm:absolute sm:left-auto sm:right-0 sm:top-full sm:mt-2 w-auto sm:w-[440px] max-w-[calc(100vw-16px)] sm:max-w-[440px] bg-white rounded-2xl border border-slate-200 shadow-2xl z-50 overflow-hidden text-slate-900 animate-fadeIn flex flex-col max-h-[85vh] sm:max-h-[580px]">
            
            {/* Header */}
            <div className="px-3.5 sm:px-5 py-3 sm:py-3.5 bg-slate-50/80 border-b border-slate-200/80 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-sm sm:text-base text-slate-900 tracking-tight">Thông báo</h3>
                {totalUnreadCount > 0 ? (
                  <span className="text-[11px] sm:text-xs font-semibold text-slate-700 bg-slate-200/80 px-2 sm:px-2.5 py-0.5 rounded-full">
                    {totalUnreadCount} mới
                  </span>
                ) : (
                  <span className="text-[11px] sm:text-xs text-slate-500 font-medium">Đã cập nhật</span>
                )}
              </div>
              ...
              <div className="flex items-center gap-1 sm:gap-1.5">
                ...
                {hasItemsInCurrentTab && (
                  <button
                    ...
                    className="px-2 sm:px-2.5 py-1 text-[11px] sm:text-xs font-semibold text-rose-600 hover:text-rose-700 hover:bg-rose-50 rounded-lg transition-colors flex items-center gap-1 cursor-pointer border border-transparent hover:border-rose-200"
            ...
            {toastMessage && (
              <div className="bg-slate-900 text-white text-xs font-medium px-4 py-2 flex items-center justify-between animate-fadeIn shrink-0">
            ...
            <div className="flex border-b border-slate-200 px-3.5 sm:px-5 gap-4 sm:gap-6 text-xs sm:text-sm font-medium bg-white shrink-0">
              <button
                type="button"
                onClick={() => setActiveTab('orders')}
                className={`py-2.5 sm:py-3 border-b-2 flex items-center gap-1.5 sm:gap-2 transition-all cursor-pointer ${
                  activeTab === 'orders'
                    ...
                    newOrders.map((ord) => {
                      ...
                      return (
                        <div
                          ...
                          className="p-3 sm:p-4 hover:bg-slate-50 transition-colors cursor-pointer group flex items-start justify-between gap-2.5 sm:gap-3 relative"
                        >
                          <div className="min-w-0 flex-1 space-y-1 sm:space-y-1.5">
                            {/* Row 1: Name + Order Code */}
                            <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                              <span className="font-bold text-slate-900 text-xs sm:text-sm truncate max-w-[130px] sm:max-w-[210px]">
                                {ord.name || ord.customerName || 'Khách vãng lai'}
                              </span>
                              <span className="text-[10px] sm:text-xs font-mono font-medium text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded">
                                #{ord.id?.slice(-6) || 'ORD'}
                              </span>
                            </div>

                            {/* Row 2: Badge + Items + Total */}
                            <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap text-[11px] sm:text-xs text-slate-600">
                              <span className={`px-1.5 sm:px-2 py-0.5 rounded text-[10px] sm:text-xs font-semibold ${srcConfig.pillClass}`}>
                          ...
                          <div className="flex flex-col items-end gap-1.5 sm:gap-2 shrink-0 pt-0.5">
                            <div className="flex items-center gap-1 sm:gap-1.5">
                              {isNew && onUpdateOrderStatus && ord.id && (
                                <button
                                  ...
                                  className="px-2 sm:px-2.5 py-1 sm:py-1.5 bg-sky-50 hover:bg-sky-600 text-sky-700 hover:text-white font-semibold rounded-lg text-[11px] sm:text-xs border border-sky-200 transition-colors flex items-center gap-1 shadow-2xs cursor-pointer"
                            ...
                            <ChevronRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-400 group-hover:text-slate-700 transition-colors mr-0.5" />
                          </div>
                        </div>
                      );
                    })
                    ...
                    unreadMessages.map((msg) => {
                      ...
                      return (
                        <div
                          ...
                          className="p-3 sm:p-4 hover:bg-slate-50 transition-colors cursor-pointer group flex items-start justify-between gap-2.5 sm:gap-3"
                        >
                          <div className="min-w-0 flex-1 space-y-1 sm:space-y-1.5">
                            <div className="flex items-center gap-1.5 sm:gap-2">
                              ...
                              <span className="text-[10px] sm:text-xs text-slate-500 font-medium ml-auto">
                                {timeText}
                              </span>
                            </div>
                            ...
                            <p className="text-[11px] sm:text-xs text-slate-700 line-clamp-2 leading-relaxed bg-slate-50 p-2 sm:p-2.5 rounded-lg border border-slate-200/70">
                              {msg.message}
                            </p>
                          </div>
                          ...
                          <div className="flex flex-col items-end gap-1.5 sm:gap-2 shrink-0 pt-0.5">
                            ...
                            <ChevronRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-400 group-hover:text-slate-700 transition-colors" />
                          </div>
                        </div>
                      );
                    })
            ...
            <div className="p-2.5 sm:p-3 bg-slate-50 border-t border-slate-200 flex items-center justify-between px-3.5 sm:px-5 shrink-0">
              {activeTab === 'orders' ? (
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="text-[11px] sm:text-xs text-slate-700 hover:text-slate-950 font-bold hover:underline transition-all inline-flex items-center gap-1 cursor-pointer"
                >
              ...
              {hasItemsInCurrentTab && (
                <button
                  type="button"
                  onClick={handleClearCurrentTab}
                  className="text-[11px] sm:text-xs text-rose-600 hover:text-rose-800 font-semibold hover:underline cursor-pointer"
                >
                  Xóa tất cả
                </button>
              )}
