...
import { Product, PaymentMethod, PaymentStatus, SellerUser } from '../types';
...
export const AdminManualOrderForm: React.FC<AdminManualOrderFormProps> = ({
  ...
  const handleSubmitManualOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const finalSellerName = isLockedSeller ? undefined : (selectedSellerName || currentSeller?.name || 'Mạnh Cường');
      ...
      // 2b. Explicitly ensure manual orders do not trigger notifications
      try {
      ...
      // 3. Auto-deduct stock for each product and selected charms, then update Firestore
      ...
      for (const it of orderItems) {
        ...
        if (pIndex > -1) {
          ...
          if (it.selectedCharm && Array.isArray(updatedCharmOptions)) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 space-y-5">
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-xs space-y-3">
              ...
              {/* Seller / Staff Dropdown (Locked for Website & Social Media) */}
              <div>
                {(() => {
                  const isLockedSeller = orderSource === 'website' || orderSource === 'mạng xã hội';
                  return (
                    <>
                      <label className="block text-xs font-bold text-slate-700 mb-1 flex items-center justify-between">
                        <span className="flex items-center gap-1.5">
                          ...
                          <span>Người bán / Nhân viên phụ trách {isLockedSeller ? '(Đã khóa)' : '*'}</span>
                        </span>
                        <span className={`text-[10px] ${isLockedSeller ? 'text-slate-400 font-medium' : 'text-slate-500 font-normal'}`}>
                      ...
                      <div className="relative">
                        ...
                        >
                          {isLockedSeller ? (
                            <option value="">🔒 Đơn {orderSource === 'website' ? 'Website' : 'Mạng xã hội'} — Không theo dõi người bán (Tự động)</option>
                          ) : (
                      ...
                      {isLockedSeller && (
                        <p className="mt-1 text-[11px] text-slate-500 flex items-center gap-1.5">
                          ...
                          <span>Đơn hàng từ <strong>{orderSource === 'website' ? 'Website' : 'Mạng xã hội'}</strong> không cần theo dõi người bán.</span>
                        </p>
                      )}
                    </>
                  );
