...
const buttonRadius = (style?: string) => style === 'pill' ? '999px' : style === 'rounded' ? '12px' : '0px';
  ...
  return <section id="hero-banner-section" className="relative w-full overflow-hidden bg-slate-950 text-white" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
    <div className="relative h-[calc(100svh-4rem)] min-h-[520px] max-h-[920px] w-full max-sm:h-[calc(100svh-3.5rem)] max-sm:min-h-[500px]">
      <AnimatePresence initial={false} custom={direction} mode="wait"><motion.div key={current.id} initial={{ opacity: 0, x: direction > 0 ? '4%' : '-4%' }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: direction > 0 ? '-4%' : '4%' }} transition={{ duration: 0.45 }} className="absolute inset-0">
        ...
        {(() => { const cta = current.button || { id: `${current.id}-button`, label: current.buttonText || 'Khám phá ngay', x: 8, y: 76, width: 24, visible: current.showButton !== false, destinationType: 'category' as const, destination: current.categoryLink || 'all', backgroundColor: current.buttonBgColor || '#fff', textColor: current.buttonTextColor || '#0f172a', style: current.buttonStyle || 'pill' as const }; if (!cta.visible) return null; const go = () => { if (cta.destinationType === 'external') window.open(cta.destination, '_blank', 'noopener,noreferrer'); else if (cta.destinationType === 'path') window.location.assign(cta.destination); else if (cta.destinationType === 'event') (onNavigateToEvent || onOpen0209Event)?.(); else (onSelectCategory || onSelectBannerCategory)?.(cta.destination || 'all'); }; return <button onClick={go} className="absolute inline-flex items-center justify-center gap-2 px-5 py-3 text-sm font-bold shadow-lg transition hover:opacity-90" style={{ left: `${cta.x}%`, top: `${cta.y}%`, width: `${cta.width}%`, backgroundColor: cta.backgroundColor, color: cta.textColor, borderRadius: buttonRadius(cta.style) }}>{cta.label}<ArrowRight className="size-4" /></button>; })()}
